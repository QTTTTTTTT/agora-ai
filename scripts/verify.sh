...
if [ "$RUN_CORE" = "1" ]; then
  ...
  (
    ...
    go test ./...
    go vet ./...
    # govulncheck is pinned, not @latest. Bump SLA + procedure
    # live in docs/RELEASE_QA_PLAYBOOK.md §0.1; CI mirror is in
    # .github/workflows/ci.yml. Keep the version string in sync
    # across all three (`git grep govulncheck@` finds them).
