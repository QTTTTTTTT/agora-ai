...
func TestPlanRepoCreateWithActionsRollsBackOnActionInsertFailure(t *testing.T) {
	...
	prepare.ExpectExec().
		WithArgs("plan-1", "NASDAQ:NVDA", "NVDA", actions[0].Market, actions[0].Exchange, actions[0].AssetClass, actions[0].InstrumentType, "buy", actions[0].PositionSide, actions[0].OpenClose, actions[0].Quantity, actions[0].Price, actions[0].Amount, actions[0].StopLoss, actions[0].TakeProfit, actions[0].Reasoning, actions[0].Confidence, sqlmock.AnyArg(), sqlmock.AnyArg(), "pending", 0, actions[0].QuoteCurrency, actions[0].SettlementCurrency, actions[0].MarginMode, actions[0].Leverage, actions[0].ContractMultiplier, actions[0].ExpiryDate, actions[0].ReduceOnly, actions[0].Sleeve, actions[0].RegimeTag, actions[0].SignalSource, actions[0].ExitReason, actions[0].Strategy).
