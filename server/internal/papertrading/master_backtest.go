...
//     calendar month. No leverage, no shorting, whole-share execution, no slippage
...
// the scheduled equal-weight rebalance. It is intentionally included
...
func (r *MasterBacktestRunner) Run(ctx context.Context, req MasterBacktestRequest) (*MasterBacktestResult, error) {
	...
	allSymbols := uniqueMasterSymbols(append(append([]string{}, req.Benchmarks...), universe...))
	...
	result := &MasterBacktestResult{
		Strategy:       "Master Team Ensemble (equal-weight, monthly whole-share rebalanced)",
		Start:          tradingDays[0],
...
func uniqueMasterSymbols(symbols []string) []string {
...
// rebalances to equal weight on the first available trading session
// of each calendar month. Symbols without a price on the rebalance
// day are dropped from that basket so missing data doesn't blow up
// the math. Execution is rounded down to whole shares; uninvested
// residual capital remains in cash.
func simulateEqualWeightMonthly(initialCapital float64, universe []string, days []time.Time, closeBy map[time.Time]map[string]float64) ([]MasterCurvePoint, []MasterOperation) {
	...
	cash := initialCapital
	rebalancedMonths := make(map[string]struct{}, len(days)/21)
	...
	for _, day := range days {
		if shouldRebalanceMonthly(day, rebalancedMonths) {
			rebalance(day)
		}
		closes := closeBy[day]
		nav := cash
...
func shouldRebalanceMonthly(day time.Time, rebalancedMonths map[string]struct{}) bool {
	key := fmt.Sprintf("%04d-%02d", day.Year(), day.Month())
	if _, ok := rebalancedMonths[key]; ok {
		return false
	}
	rebalancedMonths[key] = struct{}{}
	return true
}
